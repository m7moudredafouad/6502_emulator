...
TEST(ADCTestSuite, ZP) {
    uint8_t program[] = {Instruction::ADC_ZP, 0xF0};
    CPU cpu(program, sizeof(program));
    cpu.Execute();
...
TEST(ADCTestSuite, ZPX) {
    uint8_t program[] = {Instruction::ADC_ZPX, 0xF0};
    CPU cpu(program, sizeof(program));
    cpu.AC = 0xFF;
    cpu.Execute();
...
TEST(ADCTestSuite, ABS) {
    uint8_t program[] = {Instruction::ADC_ABS, 0xF0, 0x00};
    CPU cpu(program, sizeof(program));
    cpu.AC = 0xFF;
    cpu.SR.C = 1;
    cpu.Execute();
...
TEST(ADCTestSuite, ABSX) {
    uint8_t program[] = {Instruction::ADC_ABSX, 0x00, 0xF2};
    CPU cpu(program, sizeof(program));
    cpu.AC = 0x7F;
    cpu.Execute();
    check_registers(cpu, 4, sizeof(program), 0, 0);
    EXPECT_EQ(cpu.AC, 0x7F);
}
...
TEST(ADCTestSuite, ABSY) {
    uint8_t program[] = {Instruction::ADC_ABSY, 0x00, 0xF2};
    CPU cpu(program, sizeof(program));
    cpu.Execute();
    check_registers(cpu, 4, sizeof(program));
    EXPECT_EQ(cpu.AC, 0);
